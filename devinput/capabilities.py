...
# REFER TO: https://github.com/torvalds/linux/blob/master/include/uapi/linux/input-event-codes.h
class SynCap(enum.IntEnum):
    SYN_REPORT = 0
    SYN_CONFIG = 1
    SYN_MT_REPORT = 2
    SYN_DROPPED = 3
...
class KeyCap(enum.IntEnum):
    ...
    KEY_LINK_PHONE = 0x1BF  # AL Phone Syncing
...
class SwCap(enum.IntEnum):
    ...
    SW_USB_INSERT = 0x11  # set = USB audio device connected


class LedCap(enum.IntEnum):
    LED_NUML = 0x00
    LED_CAPSL = 0x01
    LED_SCROLLL = 0x02
    LED_COMPOSE = 0x03
...
class SndCap(enum.IntEnum):
    SND_CLICK = 0x00
    SND_BELL = 0x01
    SND_TONE = 0x02


class EvCap(enum.IntEnum):
    EV_SYN = 0x00
    EV_KEY = 0x01
    EV_REL = 0x02
    EV_ABS = 0x03
...
class Capabilities:
    def __init__(self, path):
        ...
        self._abs_cap = self._ev_cap = self._key_cap = self._led_cap = self._msc_cap = (
            self._rel_cap
        ) = self._snd_cap = self._sw_cap = None

    @property
    def abs_cap(self):
        if self._abs_cap is None:
            self._abs_cap = get_cap_set(self.abs_cap_code, AbsCap)
        return self._abs_cap

    @property
    def ev_cap(self):
        if self._ev_cap is None:
            self._ev_cap = get_cap_set(self.ev_cap_code, EvCap)
        return self._ev_cap

    @property
    def key_cap(self):
        if self._key_cap is None:
            self._key_cap = get_cap_set(self.key_cap_code, KeyCap)
        return self._key_cap

    @property
    def led_cap(self):
        if self._led_cap is None:
            self._led_cap = get_cap_set(self.led_cap_code, LedCap)
        return self._led_cap

    @property
    def msc_cap(self):
        if self._msc_cap is None:
            self._msc_cap = get_cap_set(self.msc_cap_code, MscCap)
        return self._msc_cap

    @property
    def rel_cap(self):
        if self._rel_cap is None:
            self._rel_cap = get_cap_set(self.rel_cap_code, RelCap)
        return self._rel_cap

    @property
    def snd_cap(self):
        if self._snd_cap is None:
            self._snd_cap = get_cap_set(self.snd_cap_code, SndCap)
        return self._snd_cap

    @property
    def sw_cap(self):
        if self._sw_cap is None:
            self._sw_cap = get_cap_set(self.sw_cap_code, SwCap)
        return self._sw_cap
